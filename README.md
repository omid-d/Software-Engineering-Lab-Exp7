...
استفاده از چندریختی (`Polymorphism`) به‌جای شرط باعث می‌شود که به‌جای استفاده از `if` یا `switch` برای تعیین رفتار، از کلاس‌های مختلف با پیاده‌سازی‌های مستقل استفاده کنیم. این کار خوانایی و توسعه‌پذیری کد را بالا می‌برد و از رشد بی‌رویه شرط‌ها جلوگیری می‌کند. هر کلاس مسئول رفتار خودش است و کد از اصل `Open/Closed` تبعیت می‌کند. در نتیجه تغییر یا افزودن حالت‌های جدید، بدون نیاز به دستکاری در منطق موجود انجام می‌شود.  در این سوال نیز در کلاس `Action` یک مورد می‌یابیم:

![poly1](src/main/pics/poly1.png)

که آن را به این صورت بازآرایی می‌کنیم:
...
سپس در تمام جاهایی که از این کلاس شی‌ای ساخته شده بود، کلاس فرزند متناظر را جایگزین می‌کنیم. برای مثال:

![poly2](src/main/pics/poly2.png)

بعد تغییر:

![poly3](src/main/pics/poly3.png)

در نسخه اولیه‌ی کد ، نوع عملیات مورد نظر (مثل shift یا reduce یا accept) با استفاده از شرط‌های تو در تو (if / else if) و با تکیه بر مقایسه‌ی رشته‌ای تشخیص داده می‌شد. برای نمونه اگر مقدار ستون برابر با "acc" بود، عملیات پذیرش (accept) انجام می‌شد، و در غیر این صورت با بررسی اولین حرف رشته (r یا s)، مشخص می‌شد که reduce یا shift باید انجام شود. این کار باعث می‌شد منطق مربوط به نوع عملیات به‌صورت صریح درون همین بخش از کد نوشته شود که از نظر طراحی، وابستگی بالایی ایجاد می‌کرد و خوانایی و انعطاف‌پذیری کد را کاهش می‌داد.

اما در نسخه‌ی بازآرایی‌شده ، این شرط‌ها با استفاده از چندریختی (Polymorphism) به کلاس‌های جداگانه مثل AcceptAction, ShiftAction, و ReduceAction منتقل شده‌اند. به جای اینکه در دل شرط‌ها منطق مربوط به نوع عملیات نوشته شود، هر کلاس حالا خودش مسئول پیاده‌سازی رفتار مناسب است. این کار موجب حذف شرط‌های زائد، افزایش خوانایی، قابلیت توسعه آسان‌تر و رعایت بهتر اصول شیءگرایی مانند اصل Open/Closed می‌شود. در واقع تصمیم‌گیری در مورد نوع عملیات، اکنون بر اساس نوع شیء انجام می‌شود نه شرط صریح، که دقیقاً همان کاربرد استفاده از polymorphism به‌جای شرط است.


### Separate Query from Modifier

در این بخش از کار، اصل طراحی جدا‌سازی عملیات خواندن از تغییر وضعیت یا همان `Separate Query from Modifier` را اعمال کردیم. طبق این اصل بهتر است متدی که یک مقدار را برمی‌گرداند، هیچ تغییری در وضعیت داخلی سیستم ایجاد نکند تا رفتار کد قابل‌پیش‌بینی‌تر و شفاف‌تر باشد. در نسخه قبلی، متدهایی مثل `getTemp` و `getDateAddress` نه‌تنها داده‌ای را بازمی‌گرداندند بلکه به‌صورت هم‌زمان مقادیر داخلی را هم تغییر می‌دادند، موضوعی که به‌مرور منجر به رفتارهای غیرمنتظره و اشکالات پنهان در برنامه می‌شد. برای حل این موضوع، عملیات تغییر مقدار را به متدهای مستقل مثل `increaseTemp` و `increaseDateAddress` منتقل کردیم تا مسیر تغییر و بازیابی کاملاً از هم تفکیک شود. حالا می‌توانیم با قطعیت بگوییم که هر بار `getTemp` را صدا می‌زنیم، فقط مقدار فعلی را دریافت می‌کنیم بدون اینکه چیزی در پشت‌صحنه تغییر کند، و اگر قرار است مقداری تغییر کند، این کار با صراحت و کنترل از طریق متد جداگانه انجام می‌شود. این کار نه‌تنها کد را خواناتر می‌کند بلکه تست‌نویسی را هم ساده‌تر می‌کند، چون دیگر رفتارهای پنهان در متدهای ظاهراً ساده وجود ندارد و همه چیز مستقیماً تحت کنترل است.

![sep1](src/main/pics/sep1.png)

رای حل این مشکل، دوتابع `increaseTemp` و `increaseDateAddress` را اضافه می‌کنیم و در همه جاهایی که تابع‌های get صدا زده می‌شدند، قبلشان این توابع increase را صدا می‌کنیم تا عملکرد برنامه مختل نشود و در عین حال بتوانیم query را از modifier جدا کنیم. تغییرات در کلاس‌های `CodeGenerator` و `SymbolTable` این تغییرات قابل مشاهده است.

### Self Encapsulated Field

این الگو به این معناست که وقتی شما به طور مستقیم به فیلدهای خصوصی یک کلاس دسترسی دارید، باید این دسترسی را با ایجاد getter و setter برای آن فیلدها جایگزین کنید. به این ترتیب، فقط از این متدها برای دسترسی به فیلد استفاده می‌کنید و این کار باعث می‌شود که کنترل بیشتری بر روی نحوه‌ی دسترسی و تغییرات فیلدها داشته باشید. در این سوال در کلاس `Memory`، مشاهده می‌کنیم که در این کلاس دو فیلد خصوصی lastTempIndex و lastDataAddress تابع setter ندارند و درون توابع مختلف کلاس assign می‌شوند. در نتیجه برای آن‌ها setter می‌نویسیم.

![self1](src/main/pics/self1.png)

به این شکل توابع این کلاس از setter استفاده می‌کنند.
...
در مورد بوی بد Envy Feature، ابتدا باید گفت این بو زمانی به وجود می‌آید که یک متد در یک کلاس، به جای استفاده از داده‌ها و متدهای کلاس خودش، بیشتر به داده‌ها و متدهای یک کلاس دیگر علاقه‌مند است. به عبارت دیگر، این متد "حسادت" دارد به اطلاعات و رفتارهایی که در جای دیگری قرار دارند و به جای آنکه مسئولیت‌های خود را درون کلاس خودش انجام دهد، بیش از حد درگیر کلاس دیگری می‌شود. این رفتار ناقض اصول طراحی شیءگرا است که تاکید می‌کند هر کلاس باید تنها مسئول رفتارها و داده‌های خودش باشد و تا جای ممکن خودکفا عمل کند.

در دسته‌بندی معروف بوهای بد کد که شامل پنج دسته کلی است (مانند Bloaters، Object-Orientation Abusers، Change Preventers، Dispensables، و Couplers)، بوی بد Envy Feature زیرمجموعه دسته Object-Orientation Abusers قرار می‌گیرد. این دسته به مواردی اشاره دارد که در آن‌ها استفاده نادرست یا ناکارآمد از مفاهیم پایه‌ای شیءگرایی صورت گرفته است. در مورد Envy Feature، مشکل دقیقا در این است که اصل کپسوله‌سازی و تعلق رفتار به داده‌های مرتبط در طراحی کلاس‌ها نادیده گرفته شده است.
...
9. **Coupled, Inappropriate Intimacy**
...
### سوال ششم
پلاگین formatter ابزاری است که قالب‌بندی کد را به‌صورت خودکار انجام می‌دهد و باعث می‌شود کد از نظر ظاهری یکنواخت، تمیز و منظم باشد. این ابزار برای تنظیم فاصله‌ها، تورفتگی‌ها، جای‌گذاری پرانتزها، فاصله بین عملگرها و سایر جنبه‌های ظاهری کد استفاده می‌شود. استفاده از formatter باعث می‌شود تمام اعضای تیم با یک سبک نوشتاری مشخص کد بنویسند، بدون اینکه لازم باشد به صورت دستی فرمت کد را رعایت کنند. این موضوع باعث کاهش اختلافات ظاهری بی‌مورد در Pull Requestها می‌شود، چون دیگر تفاوت‌هایی مثل فاصله اضافی یا جابجایی خطوط باعث ایجاد تغییرات غیرضروری نمی‌شود.

از نظر فنی، formatter به تیم کمک می‌کند تا تمرکز خود را به‌جای درگیری با سبک نوشتار، روی منطق، معماری و عملکرد کد متمرکز کنند. علاوه‌بر‌این، در فرآیند بازآرایی (Refactoring) که هدف آن بهبود ساختار داخلی کد بدون تغییر عملکرد آن است، ممکن است ظاهر کد به هم بریزد یا هماهنگی بین بخش‌ها از بین برود. در این مرحله، استفاده از formatter کمک می‌کند تا پس از بازآرایی، کد از نظر ظاهر نیز منظم و یکدست باقی بماند.

در واقع، formatter مکمل فرآیند refactoring است، چون پس از تغییرات ساختاری در کد، آن را از نظر ظاهری نیز به وضعیتی تمیز، خوانا و استاندارد می‌رساند. این موضوع فهم و مرور تغییرات را برای سایر اعضای تیم آسان‌تر می‌کند و از بروز مشکلات ناشی از کد نامرتب جلوگیری می‌نماید. به طور کلی، استفاده از formatter باعث افزایش کیفیت، خوانایی، نگه‌داری‌پذیری و همکاری مؤثرتر بین اعضای تیم توسعه می‌شود.
...
### تقسیم کار
در این آزمایش تلاش شد تا تسک‌ها به طور یکسان بین اعضای تیم پخش شود. در بخش اول، 7 مورد بازآرایی بین تمامی افراد تقسیم شد. همچنین سوالات نیز به طور مساوی تقسیم گشت. سوال 5ام که 10 مورد داشت هم مانند بخش اول میان اعضای گروه پخش شد.


[لینک کانبان‌برد](https://github.com/users/omid-d/projects/4)
